...
const UserReserves = ({ user, handleDeleteReserve }) => {
  ...
  return (
    <div className='userReserveTable'>
      <h3 className='text-center text-success'>Reservas</h3>
        ...
        <tbody>
          {user?.reserves?.map((res) => (
            <tr key={res.id} className='text-center'>
              ...
              <td>
                <button className='alert alert-danger m-0 px-2 py-0' onClick={() => handleDeleteReserve(res.court, res.weekday, res.id, user._id)}>
