...
const Activities = ({ setMenu, menu }) => {
  ...
  const notifyCategoriaCreada = (category) => toast.success(`Categoría ${category} creada`, { autoClose: 1000, position: 'bottom-left' });
  const notifyForm = (text) => toast.warning(`${text}`, { position: 'bottom-left', autoClose: 1000, theme: 'dark' });
  ...
  const createCategory = (e) => {
    try {
      ...
      if (categories.find(c => c.name === category.name)) {
        notifyForm('La categoría ya fue creada')

        return
      }
      categories.push(category)
      setCategories([ ...categories ]);
      notifyCategoriaCreada(category.name);
