...
const CreateUser = ({ setShowCreateUser }) => {
  ...
  return (
    ...
    <div className="rounded col-9 d-flex flex-column align-items-center p-1">

      <div className='col-12 my-3'>
        <Link to={'/'} className='btn btn-close border border-dark p-2' onClick={()=> setShowCreateUser(false)}></Link>
      </div>
      ...
      <form className="form d-flex flex-column col-8" role="form" autoComplete="on"
        ...
        <div className="input-group align-items-center justify-content-center">
          ...
          <div className="custom-control custom-checkbox mx-1">
            <input type="checkbox" name='admin' className="custom-control-input" id="customCheck1" value={true} onChange={handleChange} />
            <label className="custom-control-label mx-1 text-danger" htmlFor="customCheck1">Admin</label>
          </div>
        </div>
        ...
        <div className="input-group my-2 justify-content-center align-items-center">
          ...
          <input className="my-2 text-center border border-success w-25 rounded bg-white text-secondary p-2" type="submit" placeholder='Registrarse' onClick={handleClick} />
        </div>
      </form>
